...
const ProfileCollection = ({ profileData, loading }: { profileData: User; loading: boolean }) => {
  ...
  const { data, refetch, isRefetching } = useTapeOwnership(profileData?.wallet || '', profileData?.collection ? {} : tapeDataForOwnership);

  useEffect(() => {
    if ((data?.length, tapeDataForOwnership, profileData?.wallet)) dispatch.profileModel.updateUserCollection([profileData.wallet, tapeDataForOwnership, data]);
  }, [data, isRefetching]);
