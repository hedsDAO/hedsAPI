...
import {
  ...
  addToWaitlist,
  removeFromWaitlist,
  findRSVPByEventIdAndUserId,
} from "../../controllers/hedSpace/guestStatus";
...
router.post("/events/:eventId/rsvps", async (req, res) => {
  try {
    ...
    const existingRSVP = await findRSVPByEventIdAndUserId(
...
router.post("/events/:eventId/waitlist", async (req, res) => {
  try {
    ...
    if (waitlist) {
      ...
      return res
        .status(400)
        .json({ message: "Could not add guest to waitlist" });
    }
  } catch (error: any) {
    return res.status(500).json({ message: error.message });
  }
...
router.delete("/waitlist/:id", async (req, res) => {
  try {
    ...
    if (isRemoved) {
      ...
      return res
        .status(400)
        .json({ message: "Could not remove guest from waitlist" });
    }
  } catch (error: any) {
    return res.status(500).json({ message: error.message });
  }
